...
namespace ElympicsLobbyPackage.ExternalCommunication
{
	public class StandaloneExternalAuthenticator : IExternalAuthenticator
	{
		...
		public async UniTask<ExternalAuthData> InitializationMessage(string gameId, string gameName, string versionName)
		{
			...
			{
				device = "desktop",
				authData = _config.Capabilities is Capabilities.None
					? default
					: new AuthDataRaw()
					{
			...
			return new ExternalAuthData(null, result.device == "Mobile", (Capabilities)result.capabilities, result.environment);
		}
	}
}
